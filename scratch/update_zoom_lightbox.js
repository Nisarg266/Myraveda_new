const fs = require('fs');
const path = 'assets/myraveda-luxury.css';
...
/* Subtle Interactive Zoom Badge on hover */
.product-media-container__zoom-button::after {
  ...
  border: 1px solid rgba(201, 158, 62, 0.4);
  color: #0E281F;
  font-size: 14px;
  display: flex;
  align-items: center;
...
li[ref="media[]"]:hover .product-media-container__zoom-button::after {
  opacity: 1;
  transform: scale(1);
}

/* 2.1 HAUTE LUXE CONTROLLED LIGHTBOX MODAL (PREVENTS GIANT IMAGE BLOWOUT) */
zoom-dialog dialog,
...
dialog.dialog-zoomed-gallery__dialog {
  ...
  background: #FAF8F5 !important;
  border: 1.5px solid rgba(201, 158, 62, 0.45) !important;
...
.dialog-zoomed-gallery__dialog::backdrop {
  background: rgba(14, 40, 31, 0.75) !important;
...
button.dialog-zoomed-gallery__close-button {
  ...
  box-shadow: 0 4px 14px rgba(14, 40, 31, 0.35) !important;
...
/* Constrained Zoomed Image Container */
...
.dialog-zoomed-gallery .product-media__image {
  ...
  transform: none !important; /* Neutralize runaway 2x-5x scaling! */
...
.dialog-thumbnails-list__thumbnail {
  ...
  border: 1.5px solid rgba(201, 158, 62, 0.3) !important;
  background: #FAF8F5 !important;
...
fs.writeFileSync(path, baseCSS + luxeGridAndZoomCSS, 'utf8');
console.log('Successfully updated myraveda-luxury.css with controlled lightbox modal!');
