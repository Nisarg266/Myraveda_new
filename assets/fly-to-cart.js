...
class FlyToCart extends Component {
  ...
  connectedCallback() {
    ...
    const tryAnimate = () => {
      ...
      const sRect = this.source.getBoundingClientRect();
      const dRect = this.destination.getBoundingClientRect();
      if (sRect && dRect && (sRect.width > 0 || sRect.height > 0) && (dRect.width > 0 || dRect.height > 0)) {
        this.#animate(sRect, dRect);
        return true;
      }
      return false;
    };
